...
function renderData(page) {
  ...
  for (let index = firstIndex; index < lastIndex; index++) {
    listCatalog.innerHTML += `
        <tr>
        <td>${index + 1}</td>
        <td>${arrCatalogs[index].catalogId}</td>
        <td>${arrCatalogs[index].catalogName}</td>
        <td>${arrCatalogs[index].priority}</td>
        <td>${arrCatalogs[index].description}</td>
        <td>${arrCatalogs[index].status ? 'Active' : 'Inactive'}</td>
        <td>
            <button>Edit</button>
